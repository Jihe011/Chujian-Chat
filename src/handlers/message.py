...
from modules.recognition import ReminderRecognitionService, SearchRecognitionService, ConversationIntentRecognitor
...
class MessageHandler:
    def __init__(self, root_dir, api_key, base_url, model, max_token, temperature,
        ...
        # 发送消息回调 - 用于记录自己发送的消息ID
        self.sent_message_callback = None
...
# 微信实例
        ...
        self.preserve_format_commands = [None, '/diary', '/state', '/letter', '/list', '/pyq', '/gift', '/shopping', '/fortune', '/dream', '/story', '/joke', '/roast', '/horoscope', '/advice', '/quiz']
        ...
        self.conversation_recognitor = ConversationIntentRecognitor(self.deepseek)
        logger.info("意图识别服务已初始化")
        
        # 初始化最后回复时间
        self.last_reply_time = datetime.now()
        logger.info("对话意图时间追踪已初始化")
        ...
        # 窗口预热：启动时切换到监听用户窗口并保持
        self._warm_up_window()

    def _warm_up_window(self):
        """预热窗口：切换到目标用户窗口并恢复窗口大小，减少后续发送延迟"""
        try:
            # 尝试多种方式获取监听列表
            listen_list = None
            if hasattr(config, 'user') and hasattr(config.user, 'listen_list'):
                listen_list = config.user.listen_list
            elif hasattr(config, 'behavior'):
                try:
                    listen_list = config.get("user_settings.listen_list.value", [])
                except:
                    pass
            
            if listen_list and len(listen_list) > 0 and listen_list[0]:
                target_user = listen_list[0]
                logger.info(f"[窗口预热] 正在切换到 {target_user}")
                
                # 打开聊天窗口（最大化）
                self.wx.Navigator.open_dialog_window(friend=target_user, is_maximize=True)
                
                # 恢复窗口大小（从最大化恢复到正常大小，不是最小化）
                main_window = self.wx._main_window
                if main_window:
                    main_window.restore()
                    logger.info("[窗口预热] 已恢复窗口大小")
                
                logger.info("[窗口预热] 完成")
            else:
                logger.info("[窗口预热] 未找到监听用户，跳过")
        except Exception as e:
            logger.warning(f"[窗口预热] 失败: {e}")
            ...
            if self.debug_handler.is_debug_command(content):
                ...
                def command_callback(command, reply, chat_id):
                    try:
                        ...
                        # 使用命令响应发送方法（传递username用于表情频率控制）
                        self._send_command_response(command, reply, chat_id, username)
...
# 无论消息中是否包含链接，都将消息添加到队列
            ...
            # 私聊意图识别（非群聊）
            if not is_group and self.conversation_recognitor:
                try:
                    from modules.conversation_intent_recognition import ConversationIntent
                    intent_result = self.conversation_recognitor.recognize(
                        user_message=content,
                        context={
                            "last_reply_time": getattr(self, 'last_reply_time', None),
                            "conversation_history": [],
                            "user_online": True
                        }
                    )
                    
                    if intent_result.action == "end_conversation":
                        # 只发简短结束语，不追问
                        logger.info(f"[意图识别] 检测到结束语: {intent_result.reasoning}")
                        self._send_raw_message(intent_result.quick_reply, chat_id)
                        return None
                        
                    elif intent_result.action == "quick_reply":
                        # 先发快速回复
                        logger.info(f"[意图识别] 快速回复: {intent_result.reasoning}")
                        self._send_raw_message(intent_result.quick_reply, chat_id)
                        
                except Exception as e:
                    logger.debug(f"[意图识别] 识别失败: {e}")
            
            self._add_to_message_queue(content, chat_id, sender_name, username, is_group, is_image_recognition, video_data)
    ...
    def _add_to_message_queue(self, content: str, chat_id: str, sender_name: str,
                              username: str, is_group: bool, is_image_recognition: bool, video_data: bytes = None):
        """添加消息到队列并设置定时器"""
        # 检测消息中是否包含链接，但不立即处理
        has_link = False
        if WEBLENS_ENABLED:
        ...
        with self.queue_lock:
            ...
            if queue_key not in self.message_queues:
                ...
                # 如果新消息有视频，也保存视频数据
                if video_data:
                    self.message_queues[queue_key]['video_data'] = video_data
                if has_link:
    ...
    def _process_message_queue(self, queue_key: str):
        ...
        try:
            with self.queue_lock:
                ...
                video_data = queue_data.get('video_data')  # 获取视频数据
                ...
                if search_handled:
                    ...
                    return self._handle_text_message(processed_message, chat_id, sender_name, username, is_group, video_data)
    ...
    def _send_message_with_dollar(self, reply, chat_id, username=None):
        """以$为分隔符分批发送回复"""
        # 过滤用户标签
        ...
        if '$' in reply or '＄' in reply:
            ...
            for part in parts:
                ...
                if clean_part.strip():
                    ...
                    logger.info(f"[wx.SendMsg耗时] {send_time:.3f}秒")
                    logger.debug(f"发送消息: {clean_part[:20]}...")
                    
                    # 记录自己发送的消息ID
                    msg_id = f"{chat_id}_{datetime.now().strftime('%Y%m%d%H%M%S%f')}_{clean_part[:10]}"
                    if self.sent_message_callback:
                        self.sent_message_callback(chat_id, msg_id)

                # 发送该部分包含的表情
                for emotion_type in emotion_tags:
                    try:
                        emoji_path = self.emoji_handler.get_emoji_for_emotion(emotion_type, username)
                        if emoji_path:
                            start = time.time()
                            self.wx.SendFiles(filepath=emoji_path, who=chat_id)
                            send_time = time.time() - start
                            logger.info(f"[wx.SendFiles耗时] {send_time:.3f}秒")
                            logger.info(f"[发送耗时] 表情发送完成 - {datetime.now().strftime('%H:%M:%S.%f')[:-3]} - 类型: {emotion_type}")
                            logger.debug(f"已发送表情: {emotion_type}")
                            time.sleep(random.randint(1, 2))
                    except Exception as e:
                        logger.error(f"发送表情失败 - {emotion_type}: {str(e)}")

                logger.info(f"[发送耗时] 消息段落发送完成 - {datetime.now().strftime('%H:%M:%S.%f')[:-3]}")
            ...
            logger.info(f"[发送耗时] 非分段消息发送完成 - {datetime.now().strftime('%H:%M:%S.%f')[:-3]}")

            # 更新对话轮数计数
            if username:
                self.emoji_handler.update_conversation_count(username)
    ...
    def _send_command_response(self, command: str, reply: str, chat_id: str, username: str = None):
        ...
        if command in self.preserve_format_commands:
            ...
            # 使用正常的消息发送方式
            self._send_message_with_dollar(reply, chat_id, username)

        # 更新对话轮数计数
        if username:
            self.emoji_handler.update_conversation_count(username)

    def _handle_text_message(self, content, chat_id, sender_name, username, is_group, video_data=None):
        """处理普通文本消息
        Args:
            content: 消息内容
            chat_id: 聊天ID
        ...
        video_description = ""
        if video_data:
            logger.info("检测到视频消息，尝试理解视频内容...")
            try:
                # 保存视频到本地
                video_path = self.image_handler.save_video(video_data, chat_id)
                if video_path:
                    # 调用视频理解服务
                    video_description = self.image_handler.understand_video(video_path)
                    logger.info(f"视频理解完成: {video_description[:100]}...")
                else:
                    logger.error("视频保存失败")
            except Exception as e:
        ...
        if video_description:
            api_content = f"[用户发送了一段视频，视频内容：{video_description}]\n{api_content}"
    ...
    def add_to_queue(self, chat_id: str, content: str, sender_name: str,
        ...
        """添加消息到队列（兼容旧接口）"""
        return self._add_to_message_queue(content, chat_id, sender_name, username, is_group, False, None)
