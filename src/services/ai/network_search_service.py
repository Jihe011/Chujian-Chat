...
class NetworkSearchService:
    ...
    def get_search_model(self) -> str:
        ...
        :return: 模型名称
        """
        return "qwen-flash"  # 始终返回KouriChat模型
    ...
    def extract_web_content_direct(self, url: str) -> Optional[str]:
        ...
        try:
            ...
            # 记录API请求
            log_api_request(service_name="search_extraction", endpoint=f"{self.base_url}/chat/completions", request_data=data)
    ...
    def extract_web_content(self, url: str) -> Dict[str, str]:
        ...
        try:
            ...
            # 记录API请求
            log_api_request(service_name="web_content_extraction", endpoint=f"{self.base_url}/chat/completions", request_data=request_data)
    ...
    def search_internet(self, query: str, conversation_context: str = None) -> Dict[str, str]:
        ...
        try:
            ...
            # 记录API请求
            log_api_request(service_name="internet_search", endpoint=f"{self.base_url}/chat/completions", request_data=request_data)

            # 直接使用requests调用API而不是使用llm_service
            response = requests.post(
                f"{self.base_url}/chat/completions",
                headers=headers,
                json=request_data,  # 使用包含 enable_search 的请求数据
                timeout=120
            )
