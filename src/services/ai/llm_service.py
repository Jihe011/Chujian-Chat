...
import datetime
from datetime import datetime as dt, date, timezone  # 导入别名避免混淆
import requests
...
class LLMService:
    def __init__(self, api_key: str, base_url: str, model: str,
        ...
        # 联网搜索配置
        self.enable_search = True  # 默认开启联网搜索
    ...
    SESSION_GAP_SECONDS = 30 * 60  # 30分钟视为新会话

    def _detect_new_session(self, user_id: str) -> bool:
        """检测是否跨过会话边界（上条消息距现在 > 30分钟）"""
        ctx = self.chat_contexts.get(user_id, [])
        if not ctx:
            return False
        if 'timestamp' not in ctx[-1]:
            return False
        last_time = dt.fromisoformat(ctx[-1]['timestamp'])
        gap = (dt.now() - last_time).total_seconds()
        return gap > self.SESSION_GAP_SECONDS

    def _build_time_context(self, user_id: str) -> str:
        """构建时间上下文信息（调用前不能先加当前消息到上下文）"""
        ctx = self.chat_contexts.get(user_id, [])
        if not ctx:
            return "这是你们今天的第一次对话。"
        ...
        try:
            ...
            last_msg_time = dt.fromisoformat(last_msg['timestamp'])
            current_time = dt.now()
            ...
            if seconds < 60:
                ...
                days = seconds // 86400
                time_desc = f"距离上条消息过去了{days}天"
            
            if seconds > self.SESSION_GAP_SECONDS:
                return f"{time_desc}，现在开始的是新的对话回合，自然地开启新话题，不要强行延续之前的对话。"
    ...
    def _get_time_period(self) -> str:
        ...
        try:
            ...
            logger.error(f"获取时间段失败: {str(e)}")
            return ""
    def _get_period_activity_suggestion(self, period: str) -> str:
        """获取时间段的活动建议（弱约束）"""
        if not period:
            return ""
        suggestions = {
    ...
    def get_response(self, message: str, user_id: str, system_prompt: str, previous_context: List[Dict] = None, core_memory: str = None, enable_search: bool = None) -> str:
        """
        完整请求处理流程
        Args:
            message: 用户消息
        ...
        period_suggestion = self._get_period_activity_suggestion(time_period)
        current_hour = now.hour

        if time_period:
            time_prompt = f"当前时间是 {current_time_str}，农历{lunar_date_str}，{time_period} ({current_hour}点)。{period_suggestion}{time_context}"
        else:
            time_prompt = f"当前时间是 {current_time_str}，农历{lunar_date_str}。{time_context}"

            system_prompt: 系统提示词（人设）
            previous_context: 历史上下文（可选）
            core_memory: 核心记忆（可选）
            enable_search: 是否启用联网搜索（None时使用默认值True）
        """
        # 处理搜索参数
        ...
        # 先检测时间间隔（此时当前消息还没加入上下文，确保检测到的是上条消息的时间）
        time_context = self._build_time_context(user_id)

        # 检测会话边界
        is_new_session = self._detect_new_session(user_id)
        ...
        # 新会话时补充提示
        if is_new_session:
            time_context += "\n注意：这是新的对话回合，不要强行延续之前的话题，自然地开启新话题。"
        ...
        # 获取农历日期
        try:
            # 使用当天的午夜时间（只包含日期部分）来避免边界日期问题
            today = date.today()
            today_midnight = dt.combine(today, dt.min.time())
            lunar_date = ZhDate.from_datetime(today_midnight)
            # 获取农历年、月、日
            ...
            # 构建农历日期字符串
            lunar_date_str = f"{lunar_year}年({zodiac}年) {lunar_month_names[lunar_month-1]} {lunar_day_names[lunar_day-1]}"
            
        except Exception as e:
            logger.error(f"获取农历日期失败: {str(e)}")
            lunar_date_str = "无法获取农历日期" # 如果失败则提供一个默认值
        ...
        # 构建时间行为规则
        time_rules = f"""
当前北京时间: {current_time_str}（{time_period}，约{current_hour}点）
"""
        
        # 构建增强版时间提示词
        if time_period and period_suggestion:
            time_prompt = f"当前北京时间: {current_time_str}，农历{lunar_date_str}，{time_period}（{current_hour}点）。{time_context}{time_rules}"
        else:
            time_prompt = f"当前北京时间: {current_time_str}，农历{lunar_date_str}。{time_context}{time_rules}"
        ...
        # 构建消息列表（始终使用完整上下文，边界检测只通过提示词传递）
        session_msgs = self.chat_contexts.get(user_id, [])[-self.config["max_groups"] * 2:]
        context_messages = []
        ...
        logger.info(f"准备发送API请求 - 用户: {user_id}, 模型: {self.config['model']}, 联网搜索: {'开启' if enable_search else '关闭'}")
        ...
        for attempt in range(max_retries):
            try:
                ...
                if is_ollama:
                    ...
                    # 记录API请求到apilogs
                    log_api_request(service_name="llm_ollama", endpoint=str(self.client.base_url), request_data=request_config)
                    ...
                    # 添加联网搜索功能参数
                    if enable_search:
                        logger.info(f"为模型 {current_model} 启用联网搜索功能")
                        # 使用 extra_body 传递非标准参数
                        request_config["extra_body"] = {"enable_search": True}

                    # 记录API请求到apilogs
                    log_api_request(service_name="llm_openai", endpoint=f"{str(self.client.base_url)}chat/completions", request_data=request_config)
                ...
                # 如果是因为联网搜索导致的错误，尝试关闭搜索重试
                if enable_search and "enable_search" in str(e):
                    logger.warning("联网搜索参数可能不被当前模型支持，将在下次尝试时关闭搜索")
                    enable_search = False
                    continue
    ...
    def chat(self, messages: list, enable_search: bool = None, **kwargs) -> str:
        """
        发送聊天请求并获取回复
        ...
        try:
            ...
            logger.info(f"使用模型: {model} 发送聊天请求, 联网搜索: {'开启' if enable_search else '关闭'}")
            ...
            # 添加联网搜索功能参数
            if enable_search:
                logger.info(f"为模型 {model} 启用联网搜索功能")
                request_config["extra_body"] = {"enable_search": True}

            # 记录API请求到apilogs
            log_api_request(service_name="llm_chat", endpoint=f"{str(self.client.base_url)}chat/completions", request_data=request_config)
    ...
    def get_config(self) -> Dict:
        ...
        config_copy["enable_search"] = self.enable_search
        return config_copy  # 返回配置的副本以防止外部修改
    ...
    def set_search_enabled(self, enabled: bool) -> None:
        ...
        logger.info(f"联网搜索功能已{'开启' if enabled else '关闭'}")
