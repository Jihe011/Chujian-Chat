...
class LLMService:
    ...
    def _build_time_context(self, user_id: str) -> str:
        ...
        try:
            ...
            last_msg_time = None
            # 使用本地时间进行计算（中国时区 UTC+8）
            current_time = dt.now()

            for msg in reversed(recent_messages):
                if 'timestamp' in msg:
                    # 解析UTC时间戳（不包含时区信息）
                    last_msg_time = dt.fromisoformat(msg['timestamp'])
                    break
...
"""
        ...
        # 构建消息列表（移除时间戳字段，只保留role和content）
        context_messages = []
        for msg in self.chat_contexts.get(user_id, [])[-self.config["max_groups"] * 2:]:
