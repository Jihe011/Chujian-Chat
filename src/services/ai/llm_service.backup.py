...
# 构建时间提示词（增强版）
        ...
        if time_period:
            time_prompt = f"当前时间是 {current_time_str}，农历{lunar_date_str}，{time_period} ({current_hour}点)。{period_suggestion}。你的回复内容应与当前时间段相符。{time_context}，请根据时间的流逝调整回答内容。"
        else:
            time_prompt = f"当前时间是 {current_time_str}，农历{lunar_date_str}。你必须根据当前时间来生成你的回复内容。{time_context}，你的活动要符合当前时间段。"
