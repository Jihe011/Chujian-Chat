...
def initialize_logging():
    ...
    root_fmt = logging.Formatter('%(asctime)s - %(name)s - %(levelname)s - %(message)s')
    logging.root.setLevel(logging.INFO)
    logging.root.addHandler(logging.StreamHandler())
    logging.root.handlers[-1].setLevel(logging.WARNING)
    logging.root.handlers[-1].setFormatter(root_fmt)
    root_file = logging.handlers.RotatingFileHandler(
        logger_config.get_log_file(),
        maxBytes=10*1024*1024, backupCount=5, encoding='utf-8'
    )
    root_file.setLevel(logging.INFO)
    root_file.setFormatter(root_fmt)
    logging.root.addHandler(root_file)
    ...
    main_console.setFormatter(logging.Formatter('%(asctime)s - %(levelname)s - %(message)s'))
...
class PrivateChatBot:
    ...
    def handle_private_message(self, msg, chat_name):
        try:
            ...
            self.auto_sender.update_user_message_time(username)
...
self_sent_messages = set()  # 记录自己发送的消息ID
...
def initialize_services():
    ...
    # 设置发送消息回调 - 记录自己发送的消息ID
    def on_message_sent(chat_id, msg_id):
        global self_sent_messages
        self_sent_messages.add(msg_id)
        logger.debug(f"[自消息] 记录发送的消息ID: {msg_id[:30]}...")
    
    message_handler.sent_message_callback = on_message_sent
            ...
            for who in listen_list:
                try:
                    ...
                    for msg in new_messages:
                        ...
                        logger.debug(f"[{who}] 消息 sender={sender}, content={content[:20]}...")
                        if sender == 'self':
                        ...
                        if msg_type in ('image', 'video'):
                            ...
                            logger.info(f"[图片保存] msg_type={msg_type}, msg={msg}")
                            try:
                                save_folder = os.path.join(root_dir, 'vx文件保存', msg_type == 'image' and '图片' or '视频')
                                os.makedirs(save_folder, exist_ok=True)
                                logger.info(f"[图片保存] 调用 wx.save_media, who={who}, target={save_folder}")
                                media_path = wx.save_media(who, target_folder=save_folder)
                                logger.info(f"[图片保存] 返回路径: {media_path}")
                                if not media_path:
                                    logger.error(f"[图片保存] save_media 返回 None")
                                    content = ''
                                elif not os.path.exists(media_path):
                                    logger.error(f"[图片保存] 文件不存在: {media_path}")
                                else:
                                ...
                                logger.error(f"保存/理解媒体失败: {e}", exc_info=True)
                        
                        if not content:
                            continue
                        
                        # 检查是否是机器人自己发的消息（优先过滤）
                        sender_info = msg.get('sender', who)
                        if sender_info == 'self':
                            logger.info(f"跳过自己发送的消息 - time: {msg.get('time')}")
                            continue
                        
                        # 使用"时间+内容"组合作为消息ID，避免runtime_id复用问题
                        msg_time = msg.get('time', '')
                        if msg_time:
                            msg_id = f"{who}_{msg_time}_{content[:20]}"
                        else:
                            # time=None 时使用 index 作为后备
                            msg_index = msg.get('index', -1)
                            msg_id = f"{who}_idx{msg_index}_{content[:20]}"
                        
                        # 检查是否为自己发送的消息
                        global self_sent_messages
                        if msg_id in self_sent_messages:
                            logger.info(f"跳过自己发送的消息: {msg_id[:50]}...")
                            continue
                        
                        if msg_id in processed_messages:
                            logger.info(f"跳过已处理的消息ID: {msg_id[:50]}...")
                            continue
                        ...
                        # 调试日志
                        sender_info = msg.get('sender', who)
                        logger.info(f"[新消息] sender={sender_info}, time={msg_time}")
