...
UPDATE_ENABLED = True
UPDATE_TIMEOUT = 5
UPDATE_URLS = list(DEFAULT_UPDATE_URLS)
try:
    if os.path.exists(CONFIG_PATH):
        with open(CONFIG_PATH, 'r', encoding='utf-8') as f:
            config = json.load(f)
            cloud_api_config = config.get("cloud_api", {})
            if "enabled" in cloud_api_config:
                UPDATE_ENABLED = cloud_api_config["enabled"]
            config_urls = cloud_api_config.get("update_api_urls")
            if config_urls and isinstance(config_urls, list) and len(config_urls) > 0:
                UPDATE_URLS = config_urls
            elif cloud_api_config.get("update_api_url"):
                UPDATE_URLS = [cloud_api_config["update_api_url"]]
            if "timeout" in cloud_api_config:
                UPDATE_TIMEOUT = cloud_api_config["timeout"]
except Exception:
    pass
...
class Updater:
    ...
    def __init__(self):
        ...
        self.update_api_urls = list(UPDATE_URLS)
    ...
    def _try_fetch(self, url: str, headers: dict) -> Optional[Dict[str, Any]]:
        """尝试从单个URL获取更新信息，失败返回None"""
        try:
            import urllib3
            import certifi
    ...
    def fetch_update_info(self) -> Dict[str, Any]:
        ...
        按优先级依次尝试所有URL，全部失败则返回错误。
        """
        if not self.enabled:
            logger.debug("Update check disabled by config")
            return {"error": "Update check disabled"}

        headers = {'User-Agent': 'Chujian-Updater/1.0'}

        for url in self.update_api_urls:
            result = self._try_fetch(url, headers)
            if result is not None:
                return result
            logger.debug(f"Update URL failed: {url}")

        logger.debug("All update URLs failed")
        return {"error": "无法连接更新服务器，跳过更新检查"}
