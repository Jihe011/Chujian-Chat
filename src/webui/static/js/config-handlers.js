        ...
        for (const configKey in group) {
            ...
            if (element) {
                ...
                if (element.type === 'checkbox') {
                    ...
                    // 特殊处理：微信控制设置（WECHAT_ 开头）使用 select，不要覆盖
                    if (configKey.startsWith('WECHAT_') && element.tagName !== 'SELECT') {
                        console.log(`跳过 ${configKey} 的更新，保留 select 元素`);
                        continue;
                    }
...
function saveConfig(config) {
    ...
    // 确保视频理解API配置被正确保存
    const videoOmniEnabled = document.getElementById('VIDEO_OMNI_ENABLED');
    const videoOmniMethod = document.getElementById('VIDEO_OMNI_METHOD');
    const videoOmniFps = document.getElementById('VIDEO_OMNI_FPS');
    const videoOmniPublicUrl = document.getElementById('VIDEO_OMNI_PUBLIC_URL');
    const videoOmniApiKey = document.getElementById('VIDEO_OMNI_API_KEY');
    const videoOmniBaseUrl = document.getElementById('VIDEO_OMNI_BASE_URL');
    const videoOmniModel = document.getElementById('VIDEO_OMNI_MODEL');
    const videoOmniPrompt = document.getElementById('VIDEO_OMNI_PROMPT');
    
    if (videoOmniEnabled) config['VIDEO_OMNI_ENABLED'] = videoOmniEnabled.checked;
    if (videoOmniMethod) config['VIDEO_OMNI_METHOD'] = videoOmniMethod.value;
    if (videoOmniFps) config['VIDEO_OMNI_FPS'] = videoOmniFps.value;
    if (videoOmniPublicUrl) config['VIDEO_OMNI_PUBLIC_URL'] = videoOmniPublicUrl.value;
    if (videoOmniApiKey) config['VIDEO_OMNI_API_KEY'] = videoOmniApiKey.value;
    if (videoOmniBaseUrl) config['VIDEO_OMNI_BASE_URL'] = videoOmniBaseUrl.value;
    if (videoOmniModel) config['VIDEO_OMNI_MODEL'] = videoOmniModel.value;
    if (videoOmniPrompt) config['VIDEO_OMNI_PROMPT'] = videoOmniPrompt.value;
