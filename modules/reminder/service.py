...
class ReminderService:
    ...
    def _remind_text_generate(self, remind_content: str, sender_name: str):
        ...
        sys_prompt = sys_prompt + f"另外作为一个仿真的角色扮演者，你需要掌握一些你不一定用到的、但是十分重要的知识：{core_mem}。你的每次回应都不应该违反这些知识！"
        ...
        log_api_request(service_name="reminder", endpoint=f"{str(self.llm_service.client.base_url)}chat/completions", request_data=request_config)
