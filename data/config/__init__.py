...
class TextToSpeechSettings:
    tts_provider: str  # TTS服务提供商 (fish_audio / aliyun)
    aliyun_tts_api_key: str  # 阿里云DashScope API密钥
    aliyun_tts_model: str  # 阿里云TTS模型 (qwen3-tts-flash / qwen-tts)
    aliyun_voice: str  # 阿里云TTS音色
    aliyun_language_type: str  # 阿里云TTS语种
    tts_api_key: str  # Fish Audio API密钥 (旧配置，向后兼容)
    tts_model_id: str  # Fish Audio模型ID (旧配置，向后兼容)
    voice_dir: str  # 语音文件存储目录
    ...
    def load_config(self) -> None:
        ...
        try:
            ...
            with open(self.config_path, 'r', encoding='utf-8') as f:
                ...
                # 加载 video_omni 配置
                video_omni_data = media_data.get('video_omni', {})
                video_omni_settings = None
                if video_omni_data:
                    video_omni_settings = {
                        'enabled': video_omni_data.get('enabled', {}).get('value', False),
                        'api_key': video_omni_data.get('api_key', {}).get('value', ''),
                        'base_url': video_omni_data.get('base_url', {}).get('value', 'https://dashscope.aliyuncs.com/compatible-mode/v1'),
                        'model': video_omni_data.get('model', {}).get('value', 'qwen3.5-omni-plus'),
                        'method': video_omni_data.get('method', {}).get('value', 'vision'),
                        'public_url': video_omni_data.get('public_url', {}).get('value', 'https://video.geiniyigmmp.dpdns.org/videos/'),
                        'fps': float(video_omni_data.get('fps', {}).get('value', 2.0)),
                        'prompt': video_omni_data.get('prompt', {}).get('value', '请详细描述这个视频的内容，包括画面和音频信息')
                    }
                ...
                image_recognition=ImageRecognitionSettings(
                    ...
                    text_to_speech=TextToSpeechSettings(
                        tts_provider=text_to_speech_data.get('tts_provider', {}).get('value', 'fish_audio'),
                        aliyun_tts_api_key=text_to_speech_data.get('aliyun_tts_api_key', {}).get('value', ''),
                        aliyun_tts_model=text_to_speech_data.get('aliyun_tts_model', {}).get('value', 'qwen3-tts-flash'),
                        aliyun_voice=text_to_speech_data.get('aliyun_voice', {}).get('value', 'Cherry'),
                        aliyun_language_type=text_to_speech_data.get('aliyun_language_type', {}).get('value', 'Chinese'),
                        tts_api_key=text_to_speech_data.get('tts_api_key', {}).get('value', ''),
                        tts_model_id=text_to_speech_data.get('tts_model_id', {}).get('value', ''),
                        voice_dir=text_to_speech_data.get('voice_dir', {}).get('value', 'data/voices')
                    ),
                    video_omni=video_omni_settings
                )
...
# 认证设置
                ...
                self.auth = AuthSettings(
                    ...
                    require_password=auth_data.get('require_password', {}).get('value', True)
                )
...
# 意图识别设置
                ...
                self.wechat_settings = WeChatSettings(
                    wechat_version=wechat_data.get('wechat_version', {}).get('value', '3.9.12'),
                    wechat_lib=wechat_data.get('wechat_lib', {}).get('value', '')
                )
...
# 更新管理员密码
    ...
    # 更新认证设置（密码和登录保护）
    def update_auth_settings(self, admin_password: str = None, require_password: bool = None) -> bool:
        try:
            config_data = {
                'categories': {
                    'auth_settings': {
            ...
            if admin_password is not None:
                config_data['categories']['auth_settings']['settings']['admin_password'] = {
                    'value': admin_password
                }

            if require_password is not None:
                config_data['categories']['auth_settings']['settings']['require_password'] = {
                    'value': require_password
                }

            success = self.save_config(config_data)

            # 更新内存中的配置
            if success:
                categories = config_data['categories']['auth_settings']['settings']
                if 'admin_password' in categories:
                    self.auth.admin_password = categories['admin_password']['value']
                if 'require_password' in categories:
                    self.auth.require_password = bool(categories['require_password']['value'])

            return success
        except Exception as e:
            logger.error(f"更新认证设置失败: {str(e)}")
            return False
